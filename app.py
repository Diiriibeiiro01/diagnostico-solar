...
# --- ESTILO CSS (ROBUSTO E SEM ANIMAÇÕES) ---
st.markdown("""
    <style>
    /* Trava o fundo e as cores para evitar bugs de modo escuro */
    html, body, [data-testid="stAppViewContainer"], [data-testid="stHeader"] {
        background-color: #f4f7f6 !important;
        color: #1e354e !important;
    }
    
    /* Cabeçalho Reto e Sério */
    .header-box {
    ...
    div[data-testid="stMetricValue"] { color: #1e354e !important; font-weight: bold !important; }
...
# --- ENTRADA DE DADOS ---
st.markdown("### Dados para Diagnóstico")
nome_cliente = st.text_input("Identificação da Unidade", placeholder="Nome do cliente")

col1, col2, col3 = st.columns(3)
with col1:
    modulos = st.number_input("Número de Placas", min_value=1, step=1)
with col2:
...
if st.button("GERAR RELATÓRIO TÉCNICO"):
    if not nome_cliente:
        ...
        # Feedback de Status
        if eficiencia >= 90:
            st.success(f"Status: EXCELENTE - Sistema operando conforme esperado.")
            status_whats = "EXCELENTE"
        elif eficiencia >= 80:
            st.warning(f"Status: ALERTA - Desvio de performance. Recomendamos limpeza.")
            status_whats = "ALERTA"
        else:
            st.error(f"Status: CRÍTICO - Perda grave detectada. Necessário suporte.")
            status_whats = "CRÍTICO"

        # --- BOTÃO WHATSAPP (Ação direta para o Mateus) ---
        msg = f"Olá! Sou {nome_cliente}. Analisei minha usina na {NOME_EMPRESA} (Protocolo: {protocolo})\n\nDados:\n- Eficiência: {eficiencia:.1f}%\n- Perda: R$ {perda_rs:.2f}\n- Status: {status_whats}"
